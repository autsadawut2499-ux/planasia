...
export function MobileQuestionnaireSheet({
  ...
  return (
    <>
      ...
      />
      <aside
        className="fixed inset-x-0 bottom-0 z-[61] flex max-h-[92vh] flex-col rounded-t-2xl border border-white/10 bg-surface shadow-2xl xl:hidden"
        role="dialog"
        aria-modal="true"
        aria-label={translate("workspace.controlPanel")}
      >
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
          <div className="flex items-center gap-2">
            <ClipboardList className="h-5 w-5 text-accent" />
            <h2 className="text-sm font-semibold text-white">{translate("workspace.controlPanel")}</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-2 text-text-muted hover:bg-white/10 hover:text-white"
...
export function MobileQuestionnaireFab({ onOpen }: MobileQuestionnaireFabProps) {
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="fixed bottom-20 left-4 z-40 flex items-center gap-2 rounded-full border border-white/15 bg-surface/95 px-4 py-2.5 text-sm font-semibold text-white shadow-lg backdrop-blur-md transition hover:bg-white/10 xl:hidden"
