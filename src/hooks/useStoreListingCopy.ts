...
import { useApp, UI_LOCALE_META } from "@/context/AppContext";
...
export function useStoreListingCopy(listing: StoreListing): StoreListingCopy {
  // `locale` (content) drives the template baseline; `uiLocale` is the visitor's
  // chosen interface language and the live-translation target — so a Burmese or
  // Malay visitor gets store copy translated into their language.
  const { locale, uiLocale } = useApp();
  const base = useMemo(() => getLocalizedListing(listing, locale), [listing, locale]);

  // en/th/hi/vi have full template data already; only the wider Asian UI
  // languages need live translation.
  const needsTranslation = UI_LOCALE_META[uiLocale].content !== uiLocale;

  const [state, setState] = useState<StoreListingCopy>(() => {
    const cached = readSessionCache(listing.id, uiLocale);
  ...
  useEffect(() => {
    ...
    void fetch("/api/translate", {
      ...
      .then((data: { translations?: string[]; provider?: string }) => {
        ...
        writeSessionCache(listing.id, uiLocale, copy);
        setState(copy);
      })
      .catch(() => {
        setState({ ...base, translating: false, aiTranslated: false });
  ...
  }, [listing.id, uiLocale, base, needsTranslation]);
