...
function buildRecommendations(
  ...
  } else if (wall === "concrete-block") {
    recs.push({
      id: "wall-clay",
      messageKey: "cost.rec.downgradeWall",
      messageParams: { from: "concrete-block", to: "clay-brick" },
      estimatedSavingsThb: Math.round(overshoot * 0.06),
      action: "downgrade_materials",
      apply: (e) => setGlobalMaterials(e, { wallMaterial: "clay-brick" }),
    });
  }

  if (floor === "granite-marble" || floor === "granite") {
    recs.push({
      id: "floor-ceramic",
      messageKey: "cost.rec.downgradeFloor",
      messageParams: { from: floor, to: "ceramic-porcelain" },
      estimatedSavingsThb: Math.round(overshoot * 0.1),
      action: "downgrade_materials",
      apply: (e) => setGlobalMaterials(e, { floorMaterial: "ceramic-porcelain" }),
    });
  } else if (floor === "parquet") {
    recs.push({
      id: "floor-laminate",
      messageKey: "cost.rec.downgradeFloor",
      messageParams: { from: "parquet", to: "laminate" },
      estimatedSavingsThb: Math.round(overshoot * 0.08),
      action: "downgrade_materials",
      apply: (e) => setGlobalMaterials(e, { floorMaterial: "laminate" }),
    });
  } else if (floor === "engineered-wood") {
    recs.push({
      id: "floor-laminate-eng",
      messageKey: "cost.rec.downgradeFloor",
      messageParams: { from: "engineered-wood", to: "laminate" },
      estimatedSavingsThb: Math.round(overshoot * 0.06),
      action: "downgrade_materials",
      apply: (e) => setGlobalMaterials(e, { floorMaterial: "laminate" }),
    });
  ...
  if (roof === "clay-tile" || roof === "ceramic-roof-tile") {
    recs.push({
      id: "roof-metal",
      messageKey: "cost.rec.downgradeRoof",
      messageParams: { from: roof, to: "metal-sheet" },
      estimatedSavingsThb: Math.round(overshoot * 0.06),
      action: "downgrade_materials",
      apply: (e) => setGlobalMaterials(e, { roofMaterial: "metal-sheet" }),
    });
  } else if (roof === "concrete-roof-tile" || roof === "concrete-flat") {
    recs.push({
      id: "roof-fiber",
      messageKey: "cost.rec.downgradeRoof",
      messageParams: { from: roof, to: "fiber-cement-roof" },
      estimatedSavingsThb: Math.round(overshoot * 0.05),
      action: "downgrade_materials",
      apply: (e) => setGlobalMaterials(e, { roofMaterial: "fiber-cement-roof" }),
    });
  } else if (roof === "polycarbonate-acrylic") {
    recs.push({
      id: "roof-metal-poly",
      messageKey: "cost.rec.downgradeRoof",
      messageParams: { from: "polycarbonate-acrylic", to: "metal-sheet" },
      estimatedSavingsThb: Math.round(overshoot * 0.07),
      action: "downgrade_materials",
      apply: (e) => setGlobalMaterials(e, { roofMaterial: "metal-sheet" }),
    });
  }
