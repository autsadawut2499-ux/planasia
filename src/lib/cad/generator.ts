...
/** Minimal DXF export — floor plan room outlines; geometry in meters, labels dynamic. */
export function generatePlanDxf(doc: HousePlanDocument, options: DxfExportOptions = {}): string {
  ...
  const lines: string[] = [
    "0", "SECTION", "2", "HEADER",
    "9", "$ACADVER", "1", "AC1015",
    "9", "$INSUNITS", "70", String(DXF_INSUNITS_METERS),
    "0", "ENDSEC",
  ...
  for (const floor of doc.floorPlans) {
    for (const room of floor.rooms) {
      ...
      lines.push(
        ...
        "40", String(DXF_LABEL_TEXT_HEIGHT),
        "1", dxfRoomLabel(room.name, room.width, room.depth, unitOpts),
      );
    }
    yOffset += 20;
  }
