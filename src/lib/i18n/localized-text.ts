import { UI_LOCALE_META, type Locale, type UiLocale } from "@/lib/geo/countries";
...
/** BCP 47 lang attribute for `<html lang>` (content locale). */
export function localeHtmlLang(locale: Locale): string {
  const map: Record<Locale, string> = { en: "en", th: "th", hi: "hi", vi: "vi" };
  return map[locale];
}

/** BCP 47 lang attribute for the active interface language. */
export function uiLocaleHtmlLang(ui: UiLocale): string {
  return UI_LOCALE_META[ui]?.htmlLang ?? "en";
}

/** Human-readable language name for AI system prompts. */
export function localeName(locale: Locale | UiLocale | string): string {
  if (typeof locale === "string" && locale in UI_LOCALE_META) {
    return UI_LOCALE_META[locale as UiLocale].english;
  }
  return "English";
}

/** Instruction fragment for Gemini / AI endpoints. */
export function aiRespondInLocale(locale: Locale | UiLocale | string): string {
