...
export function buildListingDescription(
  ...
  switch (locale) {
    case "th":
      return `${style} ${project.floors} ชั้น · ${project.bedrooms} ห้องนอน · ${project.bathrooms} ห้องน้ำ · ${loc}${extraText} — คอนเซปต์บ้านที่สร้างร่วมกับ AI สำหรับนำเสนอไอเดียดีไซน์`;
    case "hi":
      return `${style} ${floorWord(project.floors, locale)} · ${project.bedrooms} बेडरूम · ${project.bathrooms} स्नानघर · ${loc}${extraText} — AI के साथ सह-निर्मित कॉन्सेप्ट हाउस प्लान`;
    case "vi":
      return `${style} · ${floorWord(project.floors, locale)} · ${project.bedrooms} phòng ngủ · ${project.bathrooms} phòng tắm · ${loc}${extraText} — Bản vẽ nhà đồng sáng tạo với AI (mức concept)`;
    default:
      return `${style} ${floorWord(project.floors, locale)} · ${project.bedrooms} bed · ${project.bathrooms} bath · ${loc}${extraText} — AI co-created concept house plan set`;
  }
}
